...
def maincode():
    ...
    plotarr=array([prsrf,pblim,pcorr,pcort,pareaf,parear,pareaa,pbcomp,pareath,pfwhm,pbrad,puni,pbmap])
...
####        plot3(px3,fw3)
    ...
    if pcorr:
        ...
        #plotKc2rel(alpharr,Kce,Kc2,area0,fsuff,figKc2rel)#
...
###        plotKcr(radlist,alphaplot,Kc2radrel,bsw,fsuff,figKcr)
    ...
    if pblim or pbcmb or parear or pbcomp or pareaa:
    ...
    if pareaa:
        ...
        for b in band:
            ...
            fitpar[:,b]=polyfit(alphabm,area_eff[:,b]/areas[b],deg=2)
            print fitpar[:,b]
            areaa_fit[:,b]=areas[b]*(fitpar[0,b]*alphabm**2 + fitpar[1,b]*alphabm + fitpar[2,b])
        
        plotareaa(alphabm,area_eff,areas,areaa_fit,fitpar,fsuff,figareaa,labareaa,unit=aunit)
...
def plotareaa(alpharr,areaeff,areas,areaa_fit,fitpar,fsuff,fignum,figlab,unit='sr'):
    ...
    plot.clf()
    for b in range(3):    
        plot.plot(alpharr,areaeff[:,b]/areas[b],c=cols[b],ls='-',label=bandStr[b])
    ...
    plot.ylabel('Relative Beam Area '+ytitu)
    plot.xlabel(r'Spectral index, $\alpha$')
    ...
    if fignum =='-':
        figname='../Docs/Paper/Figs/beamarea_alpha'+fsuff+'_'+unit
    else:
        figname='../Docs/Paper/Figs/Fig'+fignum+'_beamarea_alpha'+fsuff+'_'+unit
    plot.savefig(figname+'.png',transparent=False,dpi=300.)
    plot.savefig(figname+'.eps',transparent=False)
    ...
    yr=y1-y0
    for b in range(3):
        plot.plot(alpharr,areaa_fit[:,b]/areas[b],c=cols[b],ls=':')
        lab=r'%s: $\Omega/\Omega_0=%.3g\alpha^2'%(bandStr[b],fitpar[0,b])
        if fitpar[1,b]<0:
            lab=lab+r'%.3g\alpha'%(fitpar[1,b])
        else:
            lab=lab+r'+%.3g\alpha'%(fitpar[1,b])
        if fitpar[2,b]<0:
            lab=lab+r'%.3g'%(fitpar[2,b])
        else:
            lab=lab+r'+%.3g$'%(fitpar[2,b])
        lab = lab+' ; $\Omega_0=%.1f$ arcsec$^2$'%(areas[b])
        ...
        plot.text(x0+xr*0.05,y0+yr*0.05*(1+b),lab,color=cols[b])
    #plot.legend(loc='upper right')
    ...
    if fignum =='-':
        figname='../Docs/Paper/Figs/beamarea_alpha_withfit'+fsuff+'_'+unit
    else:
        figname='../Docs/Paper/Figs/Fig'+fignum+'_beamarea_alpha_withfit'+fsuff+'_'+unit
    plot.savefig(figname+'.png',transparent=False,dpi=300.)
    plot.savefig(figname+'.eps',transparent=False)
...
def plotKc3_va(alpharr,thetarr,Kc3,Kc2,aplot,fsuff,fignum,figlab):
    ...
    plot.ylabel(r'Surface Brightness conversion [MJy/sr per Jy]')
    plot.legend(loc='lower left',ncol=3,frameon=False)
    plot.annotate(r'$\alpha=3$',(9.e2,7000),ha='right',weight='bold',size='x-large',family='sans-serif')
    plot.yscale('log')
    plot.xscale('log')
    plot.ylim(1.e1,1.e4)
    plot.xlim(1.,1.e3)
...
def plotKc3t_va(alpharr,thetarr,Kc3t,Kc,aplot,fsuff,fignum,fignumz,figlab,figlabz):
    ...
    plot.xlabel(r'Gaussian Source FWHM [arcsec]')
    plot.ylabel(r'Total flux density conversion [Jy per Jy output by pipeline]')
    plot.legend(loc='upper left',ncol=1,frameon=False)
    plot.annotate(r'$\alpha=3$',(90,0.2),ha='right',weight='bold',size='x-large',family='sans-serif')
    plot.yscale('linear')
    plot.xscale('log')
    plot.ylim(0,10)
    plot.xlim(1,100)
    ...
    if fignum =='-':
        figname='../Docs/Paper/Figs/Kc3t_a%.1f%s'%(alpharr[aplot],fsuff)
    else:
        figname='../Docs/Paper/Figs/Fig%s_Kc3t_a%.1f%s'%(fignum,alpharr[aplot],fsuff)
    plot.savefig(figname+'.png',transparent=False,dpi=300.)
    plot.savefig(figname+'.eps',transparent=False)
    ...
    plot.ylabel(r'Total flux density conversion [Jy per Jy output by pipeline]')
    plot.legend(loc='upper left',ncol=1,frameon=False)
    plot.annotate(r'$\alpha=3$',(9,0.891),ha='right',weight='bold',size='x-large',family='sans-serif')
    plot.yscale('linear')
    plot.xscale('log')
    plot.ylim(0.89,0.93)
    plot.xlim(1e-2,10)
...
def plotKc2rel(alpharr,Kce,Kc2,area0,fsuff,fignum,figlab):
    ##############################################################
    ##plot colour correction factors
    plot.figure(23)
    plot.clf()
    ...
    plot.plot(alpharr,(Kce[:,0]/area0[0])/Kc2[:,0],'b-',label=r'PSW')
    plot.plot(alpharr,(Kce[:,1]/area0[1])/Kc2[:,1],'g-',label=r'PMW')
    plot.plot(alpharr,(Kce[:,2]/area0[2])/Kc2[:,2],'r-',label=r'PLW')
...
def plotuniantext(resList,nu0List,alphaList,KcUniAntExt,resPlot,nu0Plot,fignum,figlab):
    ...
    plot.ylim(0.85,1.05)
    plot.xlabel(r'Spectral index $(\alpha)$')
    plot.ylabel(r'Colour correction factor $K_\mathrm{Uniform}(\alpha,-1,\nu_0) \times \Omega(\nu_0)$')
