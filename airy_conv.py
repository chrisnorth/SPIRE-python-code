...
plot.figure(1)
plot.clf()
plot.plot(radListTot,airyList,'k-')
plot.plot(radListTot,apEffList,'k--')

##Define freq, wavelength and pixel width lists

nuList=arange(0.5,1.6,0.1) ##allow for 100% bandwidth
wlList=1./nuList
widList=0.5*nuList ##nominal value is for 0.5 lambda/D pixel
...
for w in range(nWid):
    ...
    convAz=airyconv(xArr,yArr,airyArr,wid,radList)
    
    apEffArr[w]=max(convAz)
    convAz=convAz/max(convAz)
    
    convArr[:,w]=convAz
    convInt=interp1d(radList,convAz,kind='cubic')
    convSort=argsort(convArr[:,w])
    convInv=interp1d(convArr[convSort,w],radList[convSort],kind='cubic')
    fwhmArr[w]=2.*convInv(0.5) ## in units of l/D

    beamArea[w]=trapz(convAz*2.*pi*radList*wlList[w]**2.,radList) ##in units of (l0/D)^2
    
    print 'FHWM = %.4f'%(fwhmArr[w])
    print 'ApEff1=%.4f'%(apEffArr[w])

    print 'Beam(FWHM)=%.4f'%(convInt(fwhmArr[w]/2.))
    
    print 'Beam Area=%.4f'%(beamArea[w])
...
##Set up output files
###########################################
fileOut='../Inputs/airy_conv.dat'
fOut=open(fileOut,'w')
line='#nu_norm, wl_norm, pix width, FWHM, Ap Eff, Beam Area\n'
fOut.write(line)
for w in range(nWid):
    line='%.3f, %.3f, %.3f, %.5f, %.5f, %.5f\n'%(nuList[w],wlList[w],widList[w],fwhmArr[w],apEffArr[w],beamArea[w])
    fOut.write(line)
fOut.close()
...
plot.clf()
for w in range(nWid):
    plot.plot(radList,convArr[:,w],label=r'%.2f $\lambda/D$'%(widList[w]))
plot.xlim(0,3)
...
plot.plot(widList,apEffArr,'ko',label='Square pixel')
plot.xlim(0,3)
...
plot.figure(6)
plot.clf()
plot.plot(widList,fwhmArr,'k-')
plot.xlabel('Pixel width')
plot.ylabel('FWHM')

plot.figure(7)
plot.clf()
plot.plot(widList,beamArea,'k-')
plot.xlabel('Pixel width')
...
apEffInt=interp1d(nuList,apEffArr,kind='cubic')
apEff0=apEffInt(1.0)
apEffNorm=interp1d(nuList,apEffArr/apEff0,kind='cubic')

##Normalised FWHM
fwhmInt=interp1d(nuList,fwhmArr,kind='cubic')
fwhm0=fwhmInt(1.0)
fhwmNorm=interp1d(nuList,fwhmArr/fwhm0,kind='cubic')

##Normalised beam area
areaInt=interp1d(nuList,beamArea,kind='cubic')
area0=areaInt(1.0)
areaNorm=interp1d(nuList,beamArea/area0,kind='cubic')
...
##Beam profile
beamInt=interp2d(radList,nuList,convArr,copy=True,kind='cubic')
