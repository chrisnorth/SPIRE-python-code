...
#Specific modules
from beam import getbeam,getbeamprofile,beam_regrid,beam_azsm,get_effnu_az,get_effnu
from beam import arcsec2sr,avgbeam,beamarea_eff,modbeam,modbeam_area,measbeam
from beam import beamarea_az,beamarea_az_nu,beamarea_az_th_nu,beamarea_az_th
...
def maincode():
    ...
    parser.add_argument('--beam',action='store',default='S',dest='beam',help='Type of beam [M(easued)|T(heoretical)|G(aussian)|E(lliptical Gaussian)]. Default=M')
    parser.add_argument('--bmap',action='store_true',default=False,dest='bmap',help='Use beam map instead of radial profile.')
    ...
    if bmap:
        fsuff='_theta_B'+beamtype+'map_Bw%d_Br%d'%(int(bwid),int(brad))
    else:
        fsuff='_theta_B'+beamtype+'prof_Br%d'%(int(brad))
    if bzlim != None:
    ...
    if bmap != True:
        #Read in spliced beam from CSV file
        radarr=arange(brad)
        nrad=radarr.size
        beam_sm=zeros((nrad,3))
        areas=zeros(3)
        for b in band:
            beam_sm[:,b]=getbeamprofile(beamtype,b,radarr,bzlim=bzlim,bzval=bzval)
            areas[b]=beamarea_az(radarr,beam_sm[:,b],brad=brad)
    else:
        #read in beam from map
        ...
        print 'Full beam areas: [%.2f , %.2f , %.2f] sq.arcsec' % (areas[0],areas[1],areas[2])
        ...
        for b in band:
            ...
            areas2[b]=beamarea_az(radarr,beam_sm[:,b])
        
        print 'Beam areas (2): [%.2f , %.2f , %.2f] sq.arcsec' % (areas2[0],areas2[1],areas2[2])
    ...
    aprec=0.0001
    nueff=get_effnu_az(radarr,beam_sm,rsrfarr,nuarr,nuc,brad,aprec=aprec,verbose=True)
    ...
    for b in band:
        print 'Band %d...'%(b)
        for a in arange(nalph):
            area_eff[a,b]=measbeam(radarr,beam_sm[:,b],nuarr,rsrfarr[:,b],nueff[b],alpharr[a])
    ...
    if parear or pbcomp:
        ...
        for b in band:
            ...
            for r in arange(nrad_a):
                ...
                areas_r[r,b]=beamarea_az(radarr,beam_sm[:,b],brad=radx)
                #print '  Radius %.1f: Area %.2f'%(radx,areas_r[r,b])
            print min(beam_sm[:,b]),max(beam_sm[:,b])
        ...
        plot.figure(30)
        plot.plot(radarr,beam_sm[:,0],'b')
        plot.plot(radarr,beam_sm[:,1],'g')
        plot.plot(radarr,beam_sm[:,2],'r')
        plot.yscale('log')
    ...
    file_tex='../Docs/Kc_Kce'+fsuff+'.tex'
    file_tex2='../Docs/Kc2_Kc2b'+fsuff+'.tex'
    ...
    ithp=array((0,28,32,37,38,39,40,41,46,56))
    nthp=ithp.size
    ...
    for b in band:
        ...
        file_Kc3_tex='../Docs/Kc3_'+bname[b]+fsuff+'.tex'
        file_Kc3t_tex='../Docs/Kc3t_'+bname[b]+fsuff+'.tex'
        #open files        
        ...
        for t in arange(nth):
            ...
            for a in arange(nalph):
                fK6at.write(',%g'%K6[a,t,b]) #conv factors
                fKc3at.write(',%g'%Kc3[a,t,b]) #conv factors
                fKc3tat.write(',%g'%Kc3t[a,t,b]) #conv factors
            fK6at.write('\n')
            fKc3at.write('\n')
            fKc3tat.write('\n')


        line='\\begin{tabular}{c|cccccccccc}\n'
        fKc3_tex.write(line)
        fKc3t_tex.write(line)
        line='& \\multicolumn{10}{c}{Source width, $\\theta_0$ (arcsec)} \\\\\n'
        fKc3_tex.write(line)
        fKc3t_tex.write(line)
