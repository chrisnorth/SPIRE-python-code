...
# Settings
outDir = "/Users/Tom/HIPE/plots/radial-beams/"
...
def loadRadialBeams(obsIn):
    ...
    for band in bands:
        ...
        simpleFitsWriter(fullBeams[band], outDir + 'RADIAL_BEAM_' + band + '.fits')
    return fullBeams
...
for obsid in obsids:
    ...
    radialBeams = loadRadialBeams(obs)
    ...
    for band in bands:
        ...
        bcenter = radialBeams[band].image.dimensions
        bcenter[0] = bcenter[0] / 2
        bcenter[1] = bcenter[1] / 2
        radialBeam = crop(radialBeams[band], int(bcenter[0] - beamSize) , int(bcenter[1] - beamSize), int(bcenter[0] + beamSize+1), int(bcenter[1] + beamSize+1))
        ...
        tempMapRadial = processHiRes(level1Corrected, band, radialBeam, wcs, mapMin, mapMax, fluxOffsetsPsrc)
        # Remove flux offset
        mapHiresPsrcNew = imageSubtract( image1=tempMap, scalar=fluxOffsetsPsrc)
        mapHiresPsrcNewR = imageSubtract( image1=tempMapRadial, scalar=fluxOffsetsPsrc)
        ...
        simpleFitsWriter(mapHiresExtdNew, outDir + str(obsid) + '_HIRES_' + band + '.fits')
        
        # Convert units to MJy/sr
        mapHiresExtdNewR = convertImageUnit(image=mapHiresPsrcNewR,beamArea=beamAreaPipArc,newUnit='MJy/sr')
        k4P = obs.calibration.getPhot().getProduct('FluxConvList')[0].meta['k4P_%s'%band].value
        k4E = obs.calibration.getPhot().getProduct('FluxConvList')[0].meta['k4E_%s'%band].value
        mapHiresExtdNewR = imageDivide(image1=mapHiresExtdNewR,scalar=k4P)
        mapHiresExtdNewR = imageMultiply(image1=mapHiresExtdNewR,scalar=k4E)
        mapHiresExtdNewR = imageAdd(image1=mapHiresExtdNewR,scalar=fluxOffsetsExtd)
        simpleFitsWriter(mapHiresExtdNewR, outDir + str(obsid) + '_HIRES_RAIDAL_' + band + '.fits')
        
        diff = imageSubtract(image1=mapHiresExtdNew, image2=mapHiresExtdNewR, ref=0)
        simpleFitsWriter(diff, outDir + str(obsid) + '_DIFF_' + band + '.fits')
