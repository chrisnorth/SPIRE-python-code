...
#Specific modules
from beam import getbeam,getbeammaps,getnewbeamprofile,comb_beam,beamprof_nu
from beam import get_effnu_az_newbeam_map
...
from calc_conv import calc_k4,calc_kc,calc_k5,calc_k4pip,calc_kcpip
...
def maincode():
    ...
    fsuff='_theta_final_newBprofBS_Br%d_Ind%.2f_%s'%(int(brad),ind,nepmod)
    if bzlim != None:
    ...
    for b in band:
        ...
        if (oldpip):
            #ignore aperture efficiency, as per old system
            Kpip[b] = calc_k4pip(alphapip,rsrfarr[:,b],nuarr,dnu,nuc[b])
            K4epip[b] = calc_k4pip(alphapip,rsrfearr[:,b],nuarr,dnu,nuc[b])/areapip[b]
            
            for a in arange(nalph):
                K4[a,b] = calc_k4pip(alpharr[a],rsrfarr[:,b],nuarr,dnu,nuc[b])

                KmonP[a,b]=K4[a,b]
                #Calculate K4 (fake-extended)    
                K4e[a,b] = calc_k4pip(alpharr[a],rsrfearr[:,b],nuarr,dnu,nuc[b])
            
                #Calculate correction factor (point & fake-extended)        
                Kc[a,b] = calc_kcpip(alpharr[a],rsrfarr[:,b],nuarr,dnu,nuc[b],alphapip)
                Kce[a,b] = calc_kcpip(alpharr[a],rsrfearr[:,b],nuarr,dnu,nuc[b],alphapip)
                
                #divide by old pipeline number
                KcolP[a,b]=K4[a,b]/Kpip[b]
        else:
            Kpip[b] = calc_k4(alphapip,rsrfarr[:,b],apfarr[:,b],nuarr,dnu,nuc[b])
            K4epip[b] = calc_k4(alphapip,rsrfearr[:,b],apfarr[:,b],nuarr,dnu,nuc[b])/areapip[b]
            ...
            for a in arange(nalph):
                ...
                #use new system for colour correction
                KcolP[a,b]=Kc[a,b]
...
#                        arange(1.,11.,0.2)*100.,[10000]))
    thetarr=(1.e-8,1.e-3,1.,10.,100.,1000.,10000)
    ...
    if pareaf:
        ...
        plotareaf(nuarr,area_bmth,area_nuind,ilim_2,avgnu_bm,avgarea_bm,ithi,fsuff,unit=aunit)
        sys.exit()
    ...
    area_pip_sr=arcsec2sr(area_pip)
    print area_eff
    print area_eff_sr
    if pareath:
        ...
        for t in arange(nth):
            for b in band:
                ...
                ##correct for rofile->map effect
                areas_th[b,t]=areas_th[b,t]*prof2map[b]
    ...
    if parear or pbcomp:
        ...
        for b in band:
            ...
            ##correct for profile->map effect
            areas_r[:,b]=areas_r[:,b]*prof2map[b]
    ...
    KpipE=zeros(3)
    Kuni=zeros((nalph,3))
    KmonE=zeros((nalph,nth,3))
    KcolEuni=zeros((nalph,3))
    KcolE=zeros((nalph,nth,3))
    KcolEtot=zeros((nalph,nth,3))
    for b in band:
        print 'band %d'%b
        KpipP2E[b]=calc_k5(alphapip,nuarr,rsrfarr[:,b],apfarr[:,b],area_bm_sr[:,b],nuc[b])/Kpip[b]
        ...
        for a in arange(nalph):
            ...
            Kc2[a,b]=K5[a,b]/Kpip[b]
            Kc2b[a,b]=Kc2[a,b]*area_eff_sr[a,b]
            ##parameters used in docs
            Kuni[a,b]=K5[a,b]
            if oldpip:
                KcolEuni[a,b]=Kuni[a,b]/K4epip[b]
            else:
                KcolEuni[a,b]=Kuni[a,b]/KpipE[b]
            ...
            for t in arange(nth):
                ...
                Kc3[a,t,b]=K6[a,t,b]/(Kpip[b])#/arcsec2sr(areas[b]))
                Kc3t[a,t,b]=K6[a,t,b]/Kpip[b]*srcarea1[t]
                ##parameters used in docs
                KmonE[a,t,b]=K6[a,t,b]
                if oldpip:
                    KcolE[a,t,b]=K6[a,t,b]/K4epip[b]
                else:
                    KcolE[a,t,b]=K6[a,t,b]/KpipE[b]
                #KcolE[a,t,b]=K6[a,t,b]/K4epip[b]
                KcolEtot[a,t,b]=KcolE[a,t,b]*srcarea1[t]
    ...
    for b in band:
        ...
        Gfact2[:,b]=(KmonP[:,b]/arcsec2sr(areas_map[b]))/Kuni[:,b]
        oldDiff[:,b]=100.*(Kce[:,b]-Kc2b[:,b])/Kc2b[:,b]
        oldDiff2[:,b]=100.*((K4e[:,b]/area_eff_sr[:,b])-Kuni[:,b])/Kuni[:,b]
        print oldDiff[:,b]
        print oldDiff2[:,b]
    ...
    fsumm.write(lsum)
    lsum='Apip=Aeff(%d): %.5g, %.5g, %.5g \n'% (alpharr[apip],area_pip[0], area_pip[1], area_pip[2])
    fsumm.write(lsum)
    lsum='K4Epip(%d) (old): %.5g, %.5g, %.5g \n'% (alpharr[apip],K4epip[0], K4epip[1], K4epip[2])
    fsumm.write(lsum)
    lsum='K4E/Beam(%d) (old) [MJy/sr per Jy]: %.5g, %.5g, %.5g \n'% (alpharr[apip],K4epip[0]/area_pip[0], K4epip[1]/area_pip[1], K4epip[2]/area_pip[2])
    fsumm.write(lsum)
    lsum='KpipE=Kuni(%d) [MJy/sr per Jy]: %.5g, %.5g, %.5g \n'% (alpharr[apip],KpipE[0]/1.e6, KpipE[1]/1.e6, KpipE[2]/1.e6)
    fsumm.write(lsum)
    lsum='KpipP2E=KpipE/Kpip: %.5g, %.5g, %.5g \n'% (KpipP2E[0], KpipP2E[1], KpipP2E[2])
    fsumm.write(lsum)
...
#    fsumm.write(lsum)
    ...
    file_tex4='../Docs/Tables/KmonP_Kuni'+fsuff+'.tex'
    file_tex5='../Docs/Tables/KcolP_KcolEuni'+fsuff+'.tex'
    file_texb='../Docs/Tables/Aeff'+fsuff+'.tex'
    ...
    fhm=open(file_fwhm,'w')
    
    ftex=open(file_tex,'w')
    ftex2=open(file_tex2,'w')
    ftex3=open(file_tex3,'w')
    ftex4=open(file_tex4,'w')
    ftex5=open(file_tex5,'w')
    ftexb=open(file_texb,'w')
    ...
    ftex3.write(line)
    line='& \\multicolumn{3}{c|}{$K_\\mathrm{monP}$} & \\multicolumn{3}{c}{$K_\\mathrm{uni}$ (MJy/sr per Jy)} \\\\\n'
    ftex4.write(line)
    line='& \\multicolumn{3}{c|}{$K_\\mathrm{colP}$} & \\multicolumn{3}{c}{$K_\\mathrm{colE}$} \\\\\n'
    ftex5.write(line)        
    line='& \\multicolumn{3}{c|}{$\Omega_\mathrm{eff}$ [arcsec$^2$]} & \\multicolumn{3}{c}{$\Omega_\mathrm{eff}$ [$\times 10^{-8}$ sr]} \\\\\n'
    ftexb.write(line)        
    ...
    for a in arange(nalph):
        ...
        line='%.1f , %.9g , %.9g , %.9g \n' % (alpharr[a],area_eff[a,0],area_eff[a,1],area_eff[a,2])
        fbm.write(line)
        line='%.1f , %.9g , %.9g , %.9g \n' % (alpharr[a],fwhm_eff[a,0],fwhm_eff[a,1],fwhm_eff[a,2])
        fhm.write(line)
        ...
        ftex3.write(line)
        line='%.1f & %.4f & %.4f & %.4f & %.4f & %.4f & %.4f \\\\ \n' % (alpharr[a],KmonP[a,0],KmonP[a,1],KmonP[a,2],Kuni[a,0]/1.e6,Kuni[a,1]/1.e6,Kuni[a,2]/1.e6)
        ftex4.write(line)
        line='%.1f & %.4f & %.4f & %.4f & %.4f & %.4f & %.4f \\\\ \n' % (alpharr[a],KcolP[a,0],KcolP[a,1],KcolP[a,2],KcolEuni[a,0],KcolEuni[a,1],KcolEuni[a,2])
        ftex5.write(line)
        line='%.1f & %.4f & %.4f & %.4f & %.4f & %.4f & %.4f \\\\ \n' % (alpharr[a],area_eff[a,0],area_eff[a,1],area_eff[a,2],area_eff_sr[a,0],area_eff_sr[a,1],area_eff_sr[a,2])
        ftexb.write(line)
    ...
    KcolE_p=Kc3
    KcolE_exp=floor(log10(KcolE_p))
    KcolE_man=Kc3_p/10.**KcolE_exp
    KcolEt_p=KcolEtot
    KcolEt_exp=floor(log10(KcolEt_p))
    KcolEt_man=KcolEt_p/10.**KcolEt_exp
    ...
    for b in band:
        ...
        file_KmonE_at='../Outputs/KmonE_'+bname[b]+fsuff+'.csv'
        file_KcolE_at='../Outputs/KmonE_'+bname[b]+fsuff+'.csv'
        file_KcolEtot_at='../Outputs/KmonEtot_'+bname[b]+fsuff+'.csv'
        file_KcolE_tex='../Docs/Tables/KmonE_'+bname[b]+fsuff+'.tex'
        file_KcolEtot_tex='../Docs/Tables/KmonEtot_'+bname[b]+fsuff+'.tex'
        ...
        fKmonEat=open(file_KmonE_at,'w')
        fKcolEat=open(file_KcolE_at,'w')
        fKcolEtat=open(file_KcolEtot_at,'w')
        fKcolE_tex=open(file_KcolE_tex,'w')
        fKcolEt_tex=open(file_KcolEtot_tex,'w')
        ...
        fKmonEat.write('#theta/alpha')
        fKcolEat.write('#theta/alpha')
        fKcolEtat.write('#theta/alpha')

        for a in arange(nalph):
            fK6at.write(',%.1f'%(alpharr[a]))
            fKc3at.write(',%.1f'%(alpharr[a]))
            fKc3tat.write(',%.1f'%(alpharr[a]))
            fKmonEat.write(',%.1f'%(alpharr[a]))
            fKcolEat.write(',%.1f'%(alpharr[a]))
            fKcolEtat.write(',%.1f'%(alpharr[a]))
        fK6at.write('\n') #end-of-line
        fKc3at.write('\n') #end-of-line
        fKc3tat.write('\n') #end-of-line
        fKmonEat.write('\n') #end-of-line
        ...
        #rows
        for t in arange(nth):
            fK6at.write('%.3g'%thetarr[t]) #row headings
            fKc3at.write('%.3g'%thetarr[t]) #row headings
            fKc3tat.write('%.3g'%thetarr[t]) #row headings
            fKmonEat.write('%.3g'%thetarr[t]) #row headings
            fKcolEat.write('%.3g'%thetarr[t]) #row headings
            fKcolEtat.write('%.3g'%thetarr[t]) #row headings
            for a in arange(nalph):
                fK6at.write(',%.9g'%K6[a,t,b]) #conv factors
                fKc3at.write(',%.9g'%Kc3[a,t,b]) #conv factors
                fKc3tat.write(',%.9g'%Kc3t[a,t,b]) #conv factors
                fKmonEat.write(',%.9g'%KmonE[a,t,b]) #conv factors
                fKcolEat.write(',%.9g'%KcolE[a,t,b]) #conv factors
                fKcolEtat.write(',%.9g'%KcolEtot[a,t,b]) #conv factors
            fK6at.write('\n')
            fKc3at.write('\n')
            fKc3tat.write('\n')
            fKmonEat.write('\n') #end-of-line
        ...
        fKcolE_tex.write(line)
        fKcolEt_tex.write(line)
        ...
        fKcolE_tex.write('$\\alpha$ ')
        fKcolEt_tex.write('$\\alpha$ ')
        for p in range(nthp):
            if thetarr[ithp[p]] < 0.1:
                ...
                fKcolE_tex.write('& $%.1f \\times 10^{%d}$ '%(th_man[ithp[p]],th_exp[ithp[p]]))
                fKcolEt_tex.write('& $%.1f \\times 10^{%d}$ '%(th_man[ithp[p]],th_exp[ithp[p]]))
                ...
                fKcolE_tex.write('& %.1f '%thetarr[ithp[p]])
                fKcolEt_tex.write('& %.1f '%thetarr[ithp[p]])
        fKc3_tex.write('\\\\\n')
        fKc3t_tex.write('\\\\\n')
        fKcolE_tex.write('\\\\\n')
        fKcolEt_tex.write('\\\\\n')
        ...
        fKcolE_tex.write('\\hline\n')
        fKcolEt_tex.write('\\hline\n')

        for a in range(nalph):
            fKc3_tex.write('%.1f '%alpharr[a])
            fKc3t_tex.write('%.1f '%alpharr[a])
            fKcolE_tex.write('%.1f '%alpharr[a])
            fKcolEt_tex.write('%.1f '%alpharr[a])
            for p in range(nthp):
                if Kc3[a,ithp[p],b] >= 10:
                    fKc3_tex.write('& $%.2f \\times 10^{%.0f}$ '%(Kc3_man[a,ithp[p],b],Kc3_exp[a,ithp[p],b]))
                ...
                if Kc3t[a,ithp[p],b] >= 10:
                    fKc3t_tex.write('& $%.2f \\times 10^{%.0f}$ '%(Kc3t_man[a,ithp[p],b],Kc3t_exp[a,ithp[p],b]))
                elif Kc3t[a,ithp[p],b] <= 0.01:
                    fKc3t_tex.write('& $%.2f \\times 10^{%.0f}$ '%(Kc3t_man[a,ithp[p],b],Kc3t_exp[a,ithp[p],b]))
                else:
                    fKc3t_tex.write('& %.4f '%Kc3t[a,ithp[p],b])
                
                if KcolE[a,ithp[p],b] >= 10:
                    fKcolE_tex.write('& $%.2f \\times 10^{%d.0f}$ '%(KcolE_man[a,ithp[p],b],KcolE_exp[a,ithp[p],b]))
                elif KcolE[a,ithp[p],b] <= 0.01:
                    fKcolE_tex.write('& $%.2f \\times 10^{%.0f}$ '%(KcolE_man[a,ithp[p],b],KcolE_exp[a,ithp[p],b]))
                else:
                    fKcolE_tex.write('& %.4f '%KcolE[a,ithp[p],b])
                
                if KcolEtot[a,ithp[p],b] >= 10:
                    fKcolEt_tex.write('& $%.2f \\times 10^{%.0f}$ '%(KcolEt_man[a,ithp[p],b],KcolEt_exp[a,ithp[p],b]))
                elif KcolEtot[a,ithp[p],b] <= 0.01:
                    fKcolEt_tex.write('& $%.2f \\times 10^{%.0f}$ '%(KcolEt_man[a,ithp[p],b],KcolEt_exp[a,ithp[p],b]))
                else:
                    fKcolEt_tex.write('& %.4f '%KcolEtot[a,ithp[p],b])
            fKc3_tex.write('\\\\\n')
            fKc3t_tex.write('\\\\\n')
            fKcolE_tex.write('\\\\\n')
            fKcolEt_tex.write('\\\\\n')
        ...
        fKcolE_tex.write('\\end{tabular}\n')
        fKcolEt_tex.write('\\end{tabular}\n')
